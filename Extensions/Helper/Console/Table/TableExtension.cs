...
namespace System
{
    public static class TableExtension
    {
        ...
        public static List<int> GetColumnWides(this IList<string> columns, IList<string[]> rows)
        {
            ...
            .Where(x => x != null)
            .Select(x => x.OToString().FullHalfLength())//得到该列每一行的字符串长度(计算中文占用两格)
            .Max())//到该列中长度最大的以列
                .ToList();
        ...
        public static string FillFormatTostring(this List<ColumnShowFormat> format, string[] objs, string delimiterStr, int columnBlankNum)
        {
            ...
            format.ForEach(f =>
            {
                ...
                string val = objs[f.Index].OToString();
                ...
                if (formatStr.IsNNull()) formatStr += $"{"".PadLeft(columnBlankNum, ' ')}";
                int alignmentStrLength = Math.Max(f.StrLength - objs[f.Index].OToString().GetChineseText().Length, 0);//对其填充空格数量
        ...
        public static string GetTopAndDwon(this List<int> columnWides, string angleStr, int columnBlankNum)
        {
            ...
            columnWides.ForEach(i =>
            {
                if (top_DownDividerdivider.IsNull()) top_DownDividerdivider += $"{angleStr}{"".PadRight(i + columnBlankNum * 2, '-')}";
